...
d <- left_join(d, ppts %>% select(Subject, ClozeScore, zClozeScore))
...
```{r}
glmm_pve_prof <- glmer(P_V ~ zClozeScore + (1 | Subject) + (1 | VideoName),
...
```{r}
glmm_mve_prof <- glmer(M_V ~ zClozeScore + (1 | Subject) + (1 | VideoName),
                       data = d_basel_l2, family = "binomial", 
                       control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
```
...
# I use this approach to plot model estimates for zClozeScore
...
eff_p <- effect("zClozeScore", glmm_pve_prof)
eff_m <- effect("zClozeScore", glmm_mve_prof)

plot(eff_p, type = "link", ylim = myylims, 
     xlab = "Proficiency\n(normalised cloze score)",
     ylab = "Log-likelihood\nof path verb",
