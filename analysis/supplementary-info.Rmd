...
plot(allEffects(glmm_adapt_beg3b))
```
...
```{r}
fm_m_prof9.expr <- "bam(Used ~ Condition +
te(Trial, ClozeScore, by = Condition) +
s(Trial, Subject, bs = 'fs') +
s(VideoName, bs = 're'),
data = dm_l2,
...
Althought the interaction terms are not significant in the model summary, we can run model comparison to see if adding the 3-way interaction with proficiency significantly improves the model:

```{r}
fm_m_prof9_no3way.expr <- "bam(Used ~ Condition + 
...
family = 'binomial')"
# load model or fit
load_or_fit("fm_m_prof9_no3way", fm_m_prof9_no3way.expr)
summary(fm_m_prof9_no3way)
```
...
itsadug::compareML(fm_m_prof9, fm_m_prof9_no3way)
```
