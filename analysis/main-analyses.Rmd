...
cutoff_prof <- quantile(ppts$ClozeScore, seq(0 , 1, length.out = 6), na.rm = TRUE)
ppts$Profic_categ <- with(ppts,
                          ifelse(ClozeScore <= cutoff_prof[3], "low prof",
                                 ifelse(ClozeScore >= cutoff_prof[4], "high prof", 
                                        "medium prof")))
# native speakers
...
plot_basel(glmm_basel, d_basel, nb_sims = 2000)  # Adjust nb_sims
```
...
```{r}
glmm_L1L2_dichot.expr <- "glmer(Used ~
    Profic_categ * VerbType * Condition_bin * cTrial +
    (1 | Subject) + (1 | VideoName),
  data = d_mod_dichot, family = 'binomial',
  control = glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=2e5)))"
# load model or fit it
load_or_fit("glmm_L1L2_dichot", glmm_L1L2_dichot.expr)
```
...
glmm_L1L2_dichot_trial.expr <- "glmer(Used ~
    Profic_categ * VerbType * Condition_bin * cTrial +
    (1 + cTrial | Subject) + (1 | VideoName),
  data = d_mod_dichot, family = 'binomial',
  control = glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=2e5)))"
# load model or fit it
load_or_fit("glmm_L1L2_dichot_trial", glmm_L1L2_dichot_trial.expr)
...
Summary of the model with just random intercepts by item and subject
(see immediately below for a more readable output):

```{r}
summary(glmm_L1L2_dichot)
```
...
Generalized linear mixed model fit by maximum likelihood (Laplace Approximation) ['glmerMod']
 Family: binomial  ( logit )
Formula: Used ~ Profic_categ * VerbType * Condition_bin * cTrial + (1 |      Subject) + (1 | VideoName)
   Data: d_mod_dichot
Control: glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e+05))

     AIC      BIC   logLik deviance df.resid 
  4186.5   4353.4  -2067.3   4134.5     4502 

Scaled residuals: 
    Min      1Q  Median      3Q     Max 
-7.3203 -0.4799  0.1597  0.5001  8.6798 

Random effects:
 Groups    Name        Variance Std.Dev.
 Subject   (Intercept) 2.856332 1.69007 
 VideoName (Intercept) 0.002567 0.05067 
Number of obs: 4528, groups:  Subject, 142; VideoName, 32

Fixed effects:
                                                                              Estimate Std. Error z value Pr(>|z|)    
(Intercept)                                                                   0.284584   0.168239   1.692   0.0907 .  
Profic_categL2hi_vs_NS                                                        0.255132   0.381779   0.668   0.5040    
Profic_categL2lo_vs_NS                                                       -0.276289   0.386477  -0.715   0.4747    
VerbTypeM_vs_P                                                               -1.132985   0.168437  -6.726 1.74e-11 ***
Condition_binPrimed_vs_Baseline                                               1.120883   0.168529   6.651 2.91e-11 ***
cTrial                                                                        0.033678   0.005692   5.917 3.28e-09 ***
Profic_categL2hi_vs_NS:VerbTypeM_vs_P                                        -0.972437   0.381805  -2.547   0.0109 *  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P                                        -0.877453   0.386447  -2.271   0.0232 *  
Profic_categL2hi_vs_NS:Condition_binPrimed_vs_Baseline                        0.541470   0.381749   1.418   0.1561    
Profic_categL2lo_vs_NS:Condition_binPrimed_vs_Baseline                        0.862252   0.386321   2.232   0.0256 *  
VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline                                0.203150   0.168149   1.208   0.2270    
Profic_categL2hi_vs_NS:cTrial                                                 0.009371   0.012187   0.769   0.4419    
Profic_categL2lo_vs_NS:cTrial                                                 0.065129   0.013231   4.923 8.54e-07 ***
VerbTypeM_vs_P:cTrial                                                        -0.033393   0.005683  -5.876 4.21e-09 ***
Condition_binPrimed_vs_Baseline:cTrial                                        0.027280   0.005683   4.800 1.59e-06 ***
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline         0.754008   0.381956   1.974   0.0484 *  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline        -0.327421   0.386274  -0.848   0.3966    
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:cTrial                                  0.026387   0.012187   2.165   0.0304 *  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:cTrial                                  0.017271   0.013208   1.308   0.1910    
Profic_categL2hi_vs_NS:Condition_binPrimed_vs_Baseline:cTrial                 0.022173   0.012183   1.820   0.0688 .  
Profic_categL2lo_vs_NS:Condition_binPrimed_vs_Baseline:cTrial                 0.029842   0.013206   2.260   0.0238 *  
VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial                         0.012215   0.005691   2.146   0.0318 *  
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial  0.021239   0.012182   1.744   0.0812 .  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial  0.004766   0.013220   0.360   0.7185    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
```{r}
summary(glmm_L1L2_dichot_trial)
```
...
Generalized linear mixed model fit by maximum likelihood (Laplace Approximation) ['glmerMod']
 Family: binomial  ( logit )
Formula: Used ~ Profic_categ * VerbType * Condition_bin * cTrial + (1 +      cTrial | Subject) + (1 | VideoName)
   Data: d_mod_dichot
Control: glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e+05))

     AIC      BIC   logLik deviance df.resid 
  4096.9   4276.6  -2020.4   4040.9     4500 

Scaled residuals: 
    Min      1Q  Median      3Q     Max 
-5.6840 -0.4488  0.0993  0.4895  5.4151 

Random effects:
 Groups    Name        Variance Std.Dev. Corr
 Subject   (Intercept) 3.651340 1.91085      
           cTrial      0.006436 0.08022  0.65
 VideoName (Intercept) 0.003517 0.05930      
Number of obs: 4528, groups:  Subject, 142; VideoName, 32

Fixed effects:
                                                                              Estimate Std. Error z value Pr(>|z|)    
(Intercept)                                                                   0.389424   0.190739   2.042  0.04119 *  
Profic_categL2hi_vs_NS                                                        0.318211   0.430617   0.739  0.45993    
Profic_categL2lo_vs_NS                                                       -0.121575   0.438094  -0.278  0.78139    
VerbTypeM_vs_P                                                               -1.270454   0.191626  -6.630 3.36e-11 ***
Condition_binPrimed_vs_Baseline                                               1.275634   0.191793   6.651 2.91e-11 ***
cTrial                                                                        0.040774   0.010053   4.056 4.99e-05 ***
Profic_categL2hi_vs_NS:VerbTypeM_vs_P                                        -0.938608   0.430697  -2.179  0.02931 *  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P                                        -0.934082   0.438086  -2.132  0.03299 *  
Profic_categL2hi_vs_NS:Condition_binPrimed_vs_Baseline                        0.628146   0.430744   1.458  0.14476    
Profic_categL2lo_vs_NS:Condition_binPrimed_vs_Baseline                        0.981667   0.438107   2.241  0.02505 *  
VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline                                0.231677   0.190106   1.219  0.22297    
Profic_categL2hi_vs_NS:cTrial                                                 0.013438   0.022087   0.608  0.54291    
Profic_categL2lo_vs_NS:cTrial                                                 0.066504   0.023244   2.861  0.00422 ** 
VerbTypeM_vs_P:cTrial                                                        -0.047318   0.010230  -4.625 3.74e-06 ***
Condition_binPrimed_vs_Baseline:cTrial                                        0.044775   0.010299   4.348 1.38e-05 ***
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline         0.843630   0.430898   1.958  0.05025 .  
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline        -0.383530   0.437685  -0.876  0.38088    
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:cTrial                                  0.030821   0.022082   1.396  0.16279    
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:cTrial                                  0.009004   0.023247   0.387  0.69852    
Profic_categL2hi_vs_NS:Condition_binPrimed_vs_Baseline:cTrial                 0.027202   0.022094   1.231  0.21825    
Profic_categL2lo_vs_NS:Condition_binPrimed_vs_Baseline:cTrial                 0.038087   0.023250   1.638  0.10139    
VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial                         0.014734   0.010011   1.472  0.14109    
Profic_categL2hi_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial  0.032007   0.022136   1.446  0.14820    
Profic_categL2lo_vs_NS:VerbTypeM_vs_P:Condition_binPrimed_vs_Baseline:cTrial -0.007837   0.023229  -0.337  0.73582    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
convergence code: 0
Model failed to converge with max|grad| = 0.00160191 (tol = 0.001, component 1)
```
...
None of the models supports the conclusion that high proficiency L2 speakers
resemble native speakers, while low proficiency L2 speakers differ from
natives.
...
are not corroborated by the GLMM analysis.
