...
to interact with the effect of L2 proficiency (`ClozeScore`) at different
...
L2 proficiency (`ClozeScore`) to vary for the different levels of `VbType_Cond`
- However, the two effects can only be additive. Our prediction ins precisely
...
In the remainder, we report the model with the interaction.
...
```{r}
# d_mod_dichot_lin <- d_l2 %>% filter(Trial <= 13)
# # recenter cTrial
# d_l2_lin$cTrial <- d_l2_lin$Trial - mean(d_l2_lin$Trial) 
```
