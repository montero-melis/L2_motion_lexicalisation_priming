...
```{r}
## TO DO: replace this function with one that plots model-estimates in logit space!
...
```{r}
f_plot(bysubj(dp, "P_V"), "Path in V")
```
...
```{r}
f_plot(bysubj(dp, "M_V"), "Manner in V")
```
...
while L2 speakers had an average probability of
`r round(inv.logit(sum(summary(glmm_mve)$coefficients[1:2,1])))`.
...
## Changes in adaptation with proficiency
