...
mypredict <- function(d_new = NULL, N = 100, d_nat = d_ns, mygam = gam_ns,
  ...
  if (compute %in% c("log-likelihood", "log-odds")) {
    cat(paste("Computing", compute))
  } else {
    stop("Compute argument has to be 'log-likelihood' or 'log-odds'")
  }
  ...
  for (n in 1:N) {
    ...
    # Compute one of two quantities depending on compute argument
    if (compute == "log-likelihood") {
      ...
      d_new$quantity <- with(d_new, log(ifelse(Used == 1, predicted, 1 - predicted)))
    } else {  # if log-odds are computed
      d_new$quantity <- with(d_new, log(ifelse(Used == 1, predicted / (1 - predicted),
                                         (1 - predicted) / predicted)))
      }
    store_values[, n] <- d_new %>% group_by(TrueSubject) %>%
      summarise(quantity = mean(quantity)) %>% pull(quantity)
  }
  # N random assignments lead to a distribution of quantity of interest / subject
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Log-odds of data sets under native model --------------------------------
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

# run and time
ptm <- proc.time()
l2_under_natmodel_LO <- mypredict(d_new = d_l2, N = 500, compute = "log-odds")
ns_under_natmodel_LO <- mypredict(d_new = d_ns, N = 500, compute = "log-odds")
proc.time() - ptm

## combine native and non-native predictions
l2_under_natmodel_LO$Group <- "L2"
ns_under_natmodel_LO$Group <- "NS"

# save(l2_under_natmodel_LO, file = "analysis/gamms/l2_under_natmodel_LO.rda")
# save(ns_under_natmodel_LO, file = "analysis/gamms/ns_under_natmodel_LO.rda")

# load(file = "analysis/gamms/l2_under_natmodel_LO.rda")
# load(file = "analysis/gamms/ns_under_natmodel_LO.rda")


# predict the log-odds of the actual native speaker data under the native model
# without reassigning the subject IDs. (This should yield the highest log-odds.)
d_ns$LO <- with(d_ns, log(ifelse(Used == 1, predicted / (1 - predicted),
                                 (1 - predicted) / predicted)))

# Store as dataframe with same shape as the two above
ns_actual_under_natmodel_LO <- d_ns %>%
  group_by(Subject, Condition, ClozeScore) %>% 
  summarise(quant50 = mean(LO)) %>% 
  mutate(quant05 = NA, quant95 = NA, Group = "ActualNS") %>%
  select(Subject:ClozeScore, quant05, quant50, quant95:Group)
...
# L2 data sorted by Proficiency (Cloze score)
l2_under_natmodel_LO %>%
  ggplot(aes(x = ClozeScore, y = quant50, colour = Condition)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ylab("Log-odds of data\n(by-subject means)")
...
# Native speakers with randomly assigned subject numbers
ns_under_natmodel_LO %>%
  ggplot(aes(x = "NULL", y = quant50, colour = Condition)) +
  geom_boxplot()


# Combined predictions
combined_preds_LO <- rbind(l2_under_natmodel_LO, ns_under_natmodel_LO,
                        data.frame(ns_actual_under_natmodel_LO))
